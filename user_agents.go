...
// RotatedUserAgent user Agents to get trick bot traffic
...
func (r *RotatedUserAgent) Shuffle() {
	s := rand.NewSource(time.Now().UnixNano())
	rand := rand.New(s)
	arr := rand.Perm(len(r.UserAgents))
	for i, a := range arr {
		r.UserAgents[i] = r.UserAgents[a]
	}
}
...
// RotatedUserAgents is an initalized version of RotatedUserAgent with 1000 UserAgents already randomly shuffled
var RotatedUserAgents *RotatedUserAgent

func init() {
	RotatedUserAgents = &RotatedUserAgent{
		mu: new(sync.RWMutex),
		UserAgents: []string{
